...
<body onLoad="init()">
	<a href="http://192.168.0.106/LoginForm">로그인폼 이동</a>
	<br />
	<P>Now Time : ${Access}</P>
...
<script>
	...
	function init() {
		...
		let now = new Date(parseInt(day[0]), parseInt(day[1]) - 1,
				parseInt(day[2]));

		for (i = 0; i < 7; i++) {
			now.setDate(now.getDate() + ((i == 0) ? 0 : 1));
			let dateDiv = document.createElement('Div');
			let month = ((now.getMonth() + 1) < 10) ? "0"
					+ (now.getMonth() + 1) : (now.getMonth() + 1);
			let date = (now.getDate() < 10) ? "0" + now.getDate() : now
					.getDate();
			dateDiv.textContent = now.getFullYear() + "-" + month + "-" + date;
			dateDiv.style.cursor = "pointer";
			dateDiv.addEventListener('click', function() {
				divClick(movie[0].mvCode, this.textContent);
			});
		...
		mvImage.style.backgroundImage = "url(/resources/images/"
				+ movie[0].mvImage + ")";
	...
	function screening(screeningData){
		...
		for(index=0; index < screeningData.length; index++){
			...
			tScreen.type = "button";
			tScreen.value = screeningData[index].mvDate.substring(11, 16) + " " + screeningData[index].mvScreen + "관";
